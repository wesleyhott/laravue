...
class LaravueServiceCommand extends LaravueCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'laravue:service {model*} 
                                {--x|mxn} 
     ...
     * @var string
     */
    protected $description = 'Makes a Model Service in Laravue standart.';
    ...
    {
        if ($this->option('mxn')) {
            return $this->info('MxN not implemented at model service');
        } else if ($this->option('view')) {
            return $this->info('view not implemented at model service');
        } else {
            $this->setStub('/service');
        }
        ...
        if ($this->option('mxn')) {
            $this->info("$date - [ {$model} ] >> {$model}Service.php");
        } else {
            $stringModel = is_array($parsedModel) ? trim($parsedModel[0]) : trim($parsedModel);
            $this->info("{$date} - [ {$stringModel} ] >> {$stringModel}Service.php");
        }
    }
